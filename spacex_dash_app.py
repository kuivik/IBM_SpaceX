...
def get_pie_chart(site):
    ...
    if site=='ALL':
        ...
        names='Launch Site',
        title='The rate of successful SpaceX launches from all the launch sites')
        return fig
    else:
        df_site_filtered=spacex_df[spacex_df['Launch Site']== site]
        filtered_df=spacex_df[spacex_df['Launch Site']== site]
        df1=filtered_df.groupby(['Launch Site','class']).size().reset_index(name='Number of launches')
        fig=px.pie(df1,values='Number of launches',names='class',title='Successful launches for the site:' + site)
        return fig
# TASK 4:
